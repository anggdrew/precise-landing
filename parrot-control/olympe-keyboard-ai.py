...
class FlightListener(olympe.EventListener):
    ...
    def send_parrot_telemetry(self, event_type, fmt, data):
        ts = time.time()
        buf = struct.pack("id" + fmt, int(event_type.value), ts, *data)
        #print(f"[DEBUG] Sending telemetry (type={event_type}, data={data}) to {UDP_TELEMETRY_IP}:{UDP_TELEMETRY_PORT}")
    ...
    def onAnyEvent(self, event, scheduler):
        ...
        if now - self.state_ts > 1 / STATE_EVENT_RATE_HZ:
            print("[EVENT] {}: {}".format(event.message.name, event.args))
            self.send_parrot_telemetry(event_type.STATE, "i", (self.state.value,))
            self.state_ts = now

    @olympe.listen_event(FlyingStateChanged(_policy="wait") | AlertStateChanged(_policy="wait") | NavigateHomeStateChanged(_policy="wait"))
...
class KeyboardCtrl(Listener):
    ...
    def _on_press(self, key):
        ...
        if char == self._ctrl_keys[Ctrl.UNBLOCK_OTHER_INPUT]:
            ...
            print("[DEBUG] Unblocked other input (socket commands allowed)")
        elif char == self._ctrl_keys[Ctrl.BLOCK_OTHER_INPUT]:
            self._other_input_blocked = True
            print("[DEBUG] Blocked other input")
...
class SockCtrl():
    ...
    def _process_sock_commands(self):
        ...
        try:
            ...
            print(f"[DEBUG] Received raw UDP buf ({len(buf)} bytes): {buf}")
        except:
            # EAGAIN
            return
...
if __name__ == "__main__":
    ...
    with FlightListener(drone):
        drone.connect()
        print("[DEBUG] Drone connected?", drone.connection_state())
        ...
        while not kb_ctrl.quit():
            ...
            #print("[DEBUG] Forcing sock_ctrl._process_sock_commands()")
            sock_ctrl._process_sock_commands() #forcing it to run
            #print("[DEBUG] Done forcing sock_ctrl._process_sock_commands()")
            ...
            if kb_ctrl.has_piloting_cmd():
                ...
                print(f"[DEBUG] Sending piloting command: {cmd}")
                drone(cmd).wait(_timeout=5).success()
