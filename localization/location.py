...
ALT_LANDING_ERROR     = 0.01 #increase value for faster landings, but may result in lower landing accuracy and erratic behaviour
YAW_LANDING_ERROR     = 0.1 #increase value for faster landings, but may result in lower landing accuracy and erratic behaviour
YAW_HEADING_ERROR     = 0.05
YAW_OUTDATED_SECS     = 1
MAX_ANCHOR_DEVIATION = cfg.SQUARE_DIAGONAL + 0.3
...
def XY_LANDING_ERROR(alt):
    err = 0.3*alt - 0.2
    return np.clip(err, 0.1, 3) #increase center value for faster landings, but may result in lower landing accuracy and erratic behaviour
...
class drone_navigator(threading.Thread):
    ...
    def _limit_acceleration(self, prev, current):
        delta = np.clip(current - prev, -self.max_acc, self.max_acc)
        return prev + delta
    ...
    def _send_command(self, roll, pitch, yaw, thr):
        ...
        #print(f"[DEBUG] (drone_navigator._send_command()): Sending UDP to {cfg.UDP_COMMANDS_IP}:{cfg.UDP_COMMANDS_PORT} - {buf}")
    ...
    def set_state(self, state):
        print(f"[DEBUG] Drone state updated to {state}")
        self.state = state
    ...
    def land_drone(self, target_pos, target_yaw, in_pos, in_yaw):
        ...
        elif xy_ready and yaw_ready: #yaw_ready can delete to not require perfect yaw for landing, but yaw still needed for navigation
            now = time.time()
    ...
    def execute_action(self, in_pos, in_yaw):
        ...
        if action_type == action_type.TAKEOFF:
            if self.state == drone_state.LANDED and (now - self.lastTakeoffTime) > self.takeoffCooldown:
                self.takeoff_drone()
...
def create_parrot_sock():
    ...
    print(f"[DEBUG] Listening for UDP telemetry on {cfg.UDP_TELEMETRY_IP}:{cfg.UDP_TELEMETRY_PORT}")
    sock.setblocking(0)
...
def replace_unreasonable_anchors(loc): #incomplete implementation: replacing with median not necessarily the best action
    ...
    for a in anchors: 
        dist_diff = np.abs(a['dist']['dist'] - median_dist)
        if dist_diff > MAX_ANCHOR_DEVIATION:
            print(f"Replacing anchor {a['addr']} (deviation {dist_diff:.2f} m) with median position")
            a['dist']['dist'] = median_dist
        ...
        for dwm_fd in dwm_fds:
            ...
            if is_dwm_location_reliable(loc):
                ...
                if is_dwm_location_reasonable(loc):
                    ...
                    dwm_locs[dwm_fd] = remove_unreasonable_anchors(loc) #swappable with replace_unreasonable_anchors(loc) if replace func defined properly
...
if __name__ == '__main__':
    ...
    if args['--data-source'] == 'sock':
        ...
        sys.exit(-1)
    print(f"localization started, source = {DWM_DATA_SOURCE}")

    actions = action_parser()
    res = actions.parse(args['--mission'], args['--loop'])
    if not res:
        print("[ERROR] Could not parse --mission argument.")
        sys.exit(-1)
    print("[DEBUG] Parsed actions:", actions.actions)
    ...
    while navigator.in_progress():
        ...
        if parrot_data is not None and without_dwm_data < UNRELIABLE_UPDATES:
            print(f"[DEBUG] Parrot data received: {parrot_data}")
            without_dwm_data += 1
