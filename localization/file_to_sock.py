...
def send_dwm_data(dwm_sock, loc):
    ...
    pos = loc['pos']
    assert isinstance(loc['ts'], float), "Timestamp must be a float"

    # (x, y, z, pos_qf, pos_valid, ts, nr_anchors)
    x_mm, y_mm, z_mm = pos['coords']
    qf = int(pos['qf'])  # ensure it's int
    valid = int(bool(pos['valid']))  # only 0 or 1
    ts = float(loc['ts'])
    nr_anchors = len(loc['anchors'])

    # Debug print:
    print("[DEBUG] Raw values:")
    print(f"x_mm={x_mm}, y_mm={y_mm}, z_mm={z_mm}")
    print(f"qf={qf}, valid={valid}, ts={ts}, anchors={nr_anchors}")

    # Check for out-of-range values
    for name, value in zip(["qf", "valid"], [qf, valid]):
        if not (-32768 <= value <= 32767):
            raise ValueError(f"[ERROR] {name}={value} is out of 'h' range!")
...
def parse_log_file(dwm_sock, parrot_sock, data_file):
    ...
    while True:
        ...
        ts = float(line[line.find(":")+1 : line.find("[")])
